class ZonaSchema(object):
    ...
    def _serialize_collection_pages(cls, pagination, latlng):
        return {
            "zonas": [cls._serialize(item, latlng) for item in pagination.items],
    ...
    def _serialize_collection(cls, collection, latlng):
        return {
            "zonas": [cls._serialize(item, latlng) for item in collection],
        }

    @classmethod
    def _serialize(cls, obj, latlng):
        diccionario ={attr.name: getattr(obj, attr.name) for attr in obj.__table__.columns}
        diccionario.update(ZonaSchema._serialize_coordenadas(obj.coordenadas, latlng))
        return diccionario
