<template>
  <div>
    <div class="mb-3 map-size">
      <l-map :zoom="zoom" :center="center">
        <l-tile-layer :url="url"></l-tile-layer>
        <div v-for="(zona, index) in zonas" :key="`zonas.${index}`">
          <l-polygon
            :lat-lngs="zona.coordenadas"
            :color="zona.color"
            :fill="true"
            :fillColor="zona.color"
          >
            <l-popup>
              <h4>{{ zona.nombre }}</h4>
            </l-popup>
            <l-tooltip>¡Haga click para ver la informacion!</l-tooltip>
          </l-polygon>
        </div>
      </l-map>
    </div>
    <div class="row justify-content-center">
      <div class="col-lg-6 mt-5">
        <h2 class="mb-4 text-center">
          <i class="bi bi-map-fill zona-size"></i> Zonas
        </h2>
        <div v-for="(zona, index) in zonas" :key="`zonas.${index}`">
          <div class="card mt-2 p-2 border-gradient">
            <div class="row">
              <div class="col-8">
                <div class="row">
                  <h4>{{ zona.nombre }}</h4>
                </div>
                <div class="row">
                  <div class="col">
                    <router-link
                      :to="{ name: 'Zona Detalle', params: { id: zona.id} }"
                      class="btn btn-info text-start"
                      ><i class="bi bi-arrow-right-square"></i> ver
                      zona</router-link
                    >
                  </div>
                </div>
              </div>
              <div class="col-4">
                <div class="color-zona" :style="`--color: ${zona.color}`"></div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import {
  LMap,
  LTileLayer,
  LPolygon,
  LPopup,
  LTooltip,
} from "@vue-leaflet/vue-leaflet";

export default {
  name: "Mapa",
  components: {
    LMap,
    LTileLayer,
    LPolygon,
    LPopup,
    LTooltip,
  },
  data() {
    return {
      url: "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",
      center: [-34.921302, -57.954297],
      zoom: 12,
      zonas: [],
    };
  },
  created() {
    fetch(
      "https://admin-grupo8.proyecto2021.linti.unlp.edu.ar/api/zonas-inundables/not-pages-latlng"
    )
      .then((response) => {
        return response.json();
      })
      .then((json) => {
        this.zonas = json.zonas;
      })
      .catch((e) => {
        console.log(e);
      });
  },
};
</script>

<style scoped>
.map-size {
  width: 100%;
  height: 500px;
  border: 1px solid #000000;
}

.color-zona {
  --color: #ffffff;
  height: 100%;
  width: 100%;
  background-color: var(--color);
  border: 2px solid #000000;
}

.border-gradient {
  border: 1px !important;
  border-style: solid !important;
  border-image: linear-gradient(
      to right,
      rgba(214, 30, 178, 1) 0%,
      rgba(255, 231, 26, 1) 50%,
      rgba(0, 200, 255, 1) 100%
    )
    1 !important;
}
</style>